        ...
        if (alerts.length < 3) {
          try {
            const response = await base44.integrations.Core.InvokeLLM({
              prompt: `You are Nudge, a calm financial coach. Give ONE actionable financial recommendation based on this context. Be encouraging, specific, and under 2 sentences. Never shame.

Financial context: ${buildContextString(ctx)}

Return just the recommendation text.`,
            });
            alerts.push({ id: 'ai-rec', type: 'ai', icon: Sparkles, title: 'Nudge recommends', desc: response, color: 'text-primary bg-primary/10', read: false });
          } catch {}
        }
